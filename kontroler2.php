<?php
    ...
    class KONTROLER{
        ...
        public function register($kd_user,$name,$username,$password,$confirm,$level,$redirect){
            ...
            if($rows == 0){
                ...
                if($password == $confirm){
                    ...
                    $sql = "INSERT INTO table_user VALUES('$kd_user','$name','$username','$password','$level')";
                    $query   = mysqli_query($con,$sql);
                    if($query){
        ...
        public function selectWhereOrder($table,$where,$whereValues,$where1,$where2){
            global $con;
            $sql = "SELECT * FROM $table WHERE $where = '$whereValues' ORDER BY `$where1`.`$where2` DESC LIMIT 4";
            $query = mysqli_query($con,$sql);
            $data = [];
            while($bigData = mysqli_fetch_assoc($query)){
                $data[] = $bigData;
        ...
        public function selectWhereOrder2($table,$where,$whereValues,$wheree,$whereValuess,$where1,$where2){
            global $con;
            $sql = "SELECT * FROM $table WHERE $where = '$whereValues' AND $wheree = '$whereValuess' ORDER BY `$where1`.`$where2` DESC";
            $query = mysqli_query($con,$sql);
            $data = [];
            while($bigData = mysqli_fetch_assoc($query)){
                $data[] = $bigData;
        ...
        public function selectOrder($table,$field){
            global $con;
            $sql = "SELECT * FROM $table ORDER BY `$table`.`$field` DESC";
            $query = mysqli_query($con,$sql);
            $data = [];
            while($bigData = mysqli_fetch_assoc($query)){
                $data[] = $bigData;
        ...
        public function selectDescLimit($table,$posisi,$batas){
            global $con;
            $sql = "SELECT * FROM $table ORDER BY id DESC LIMIT $posisi,$batas";
            $query = mysqli_query($con,$sql);
            $data = [];
            while($bigData = mysqli_fetch_assoc($query)){
                $data[] = $bigData;
